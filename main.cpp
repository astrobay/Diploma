...
void create_L_approximation(vec4 *koeffs, vec4 *res, int numFunctions, int numSamples);
...
//Create compute shader
	...
	glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(structSamples), &structSamples[0], 
		GL_STATIC_DRAW);
	...
	vec4 *res = (vec4 *)glMapBuffer(GL_SHADER_STORAGE_BUFFER, GL_WRITE_ONLY);
	vec4 koeffsL[numFunctions];

	create_L_approximation(koeffsL,res,numFunctions,numSamples);

    Model ourModel("spheres/sphere1.obj");
...
void create_L_approximation(vec4 *koeffs, vec4 *res, int numFunctions, int numSamples)
{
    ...
    for (int i=0; i<numSamples; i++) {
        for (int j=0; j<numFunctions; j++) {
            ...
            cout << res[i*numFunctions+j].x << " | " << res[i*numFunctions+j].y 
                << " | " << res[i*numFunctions+j].z << "\n";
        }
        cout << "\n";
    }
